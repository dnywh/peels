...
type PeelsLogoTone = "quaternary" | "emptyState";

const StyledPeelsLogo = styled.svg<{ $tone?: PeelsLogoTone }>`
  color: ${theme.colors.logo.primary};

  ${({ $tone }) => $tone === "quaternary" && quaternaryStyles}
  ${({ $tone }) => $tone === "emptyState" && emptyStateStyles}
`;

type PeelsLogoProps = Omit<SVGProps<SVGSVGElement>, "color"> & {
  size?: number;
  color?: PeelsLogoTone;
};

function PeelsLogo({ size = 32, color, ...props }: PeelsLogoProps) {
  const tone = color;
