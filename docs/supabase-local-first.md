...
Use `npm run supabase:status`. Peels uses `54331`–`54334` so it can sit beside other local Supabase stacks.
...
After `npm run supabase:start`, open Mailpit at `http://127.0.0.1:54334` to inspect local auth emails.
...
When `NEXT_PUBLIC_SUPABASE_URL` points at local Supabase, avatar and listing-photo uploads stay in local Storage. Repo-tracked bucket media is uploaded only by `npm run seed:local-media`, not by hosted Supabase deploys.
